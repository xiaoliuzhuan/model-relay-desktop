...
Model Relay Desktop は、Windows と macOS 向けのローカルプロキシベースの IDE 固定モデルプロバイダーソリューションです。
...
- 🔄 **モデルマッピングアーキテクチャのリファクタリング** - 「1対1マッピング」から「統一マッピングモデル」アーキテクチャへ変更
  - trae側は統一されたマッピングモデルIDを使用し、Model Relay Desktop は設定グループで実際のバックエンドモデルを切り替え
  - プロキシサーバーはモデルIDマッピングとローカル代理認証の検証をサポート
  - グローバル設定でマッピングモデルIDとクライアントアクセスキーの設定をサポート
...
-> [Mac OS スクリプト起動方法](README_macOS_cli.md)
