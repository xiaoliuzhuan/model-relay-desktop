  ...
  <div class="form-control w-full">
    ...
    >
      ...
      <div 
        ...
        :class="[
          size === 'xs' ? 'w-4' : 'w-5',
          error ? 'text-error/70' : 'text-slate-400 group-hover:text-primary/60 group-focus-within:text-primary'
        ]"
      >
        <slot name="leading">
          <svg v-if="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-full h-full">
            <path :d="icon" />
          </svg>
        </slot>
      </div>
      ...
      <div v-if="hasActionArea" class="absolute right-0 top-0 bottom-0 flex items-center pr-2">
        ...
        <div class="flex items-center gap-1">
          ...
          >
            <slot name="trailing">
              <svg v-if="trailingIcon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-full h-full">
                <path :d="trailingIcon" />
              </svg>
            </slot>
          </div>
